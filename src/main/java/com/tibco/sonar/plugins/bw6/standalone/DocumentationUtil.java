...
import com.tibco.sonar.plugins.bw6.standalone.RulesInfo.DocumentationException;
...
public class DocumentationUtil {
	...
	public static void main(String[] args) throws Exception {
		System.out.println("***** CALLED UTILS *****");
	...
	private final static void writeRulesFile() throws Exception {
		FileOutputStream fos = new FileOutputStream("docs/rules/RULES.md");
		...
		ProcessRuleDefinition.getCheckList().stream().sorted((o1, o2) -> o1.getRuleKeyName().compareTo(o2.getRuleKeyName()))
		    .forEach(rule -> {
			    try {
				    ...
				    String htmldocs = info.getHTMLDocForRule(name);
	...
	private final static void writeRulesFile(AbstractCheck check) throws DocumentationException, IOException {
		...
		FileOutputStream fos = new FileOutputStream("docs/rules/" + check.getRuleKeyName() + ".md");
		PrintStream ps = new PrintStream(fos);


		String htmldocs = info.getHTMLDocForRule(check.getRuleKeyName());
		...
		ps.println("### Using within SonarQube");
		ps.println("");
