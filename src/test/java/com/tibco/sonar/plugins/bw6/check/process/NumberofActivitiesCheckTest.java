 ...
 * @author avazquez
 */
public class NumberofActivitiesCheckTest {
    
    private static ProcessSource source;
    
    public NumberofActivitiesCheckTest() {
    }
...
"</bpws:process>");
    ...
    public void testValidate() {
        ...
        NumberofActivitiesCheck instance = new NumberofActivitiesCheck();
        NumberofActivitiesCheck spyInstance = Mockito.spy(instance);
        doNothing().when(spyInstance).reportIssueOnFile(any());    
        spyInstance.setMaxActivities(10);
        spyInstance.validate(source);        
        Mockito.verify(spyInstance,never()).reportIssueOnFile(anyString());
        spyInstance.setMaxActivities(1);
        spyInstance.validate(source);        
