 ...
 * @author avazquez
 */
public class NumberofServicesCheckTest {
    
    private static ProcessSource source;
    
    public NumberofServicesCheckTest() {
    }
...
"</bpws:process>");
    ...
    public void testValidate() {
        ...
        NumberofServicesCheck instance = new NumberofServicesCheck();
        NumberofServicesCheck spyInstance = Mockito.spy(instance);
        doNothing().when(spyInstance).reportIssueOnFile(any());        
        spyInstance.setMaxServices(10);
        spyInstance.validate(source);        
        Mockito.verify(spyInstance,never()).reportIssueOnFile(anyString());
        spyInstance.setMaxServices(0);
        spyInstance.validate(source);        
